<script>
	...
	export let numKeys = 128;
	export let startKey = 0;
	export let activeKeys = new Array(numKeys).fill(false);
	export let enabledKeys = new Array(numKeys).fill(true);
	export let mask = new Array(numKeys).fill(true);
...
<div class="scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent overflow-x-auto bg-slate-950 select-none" bind:this={container}>
	{#if showMaskButtons}
		<div class="h-4 flex flex-row" style:height="{(WKEY_W + WKEY_S) * 7/12}rem">
			{#each { length: numKeys } as _, n}
				<div class="grid place-items-center aspect-square shrink-0" style:width="{(WKEY_W + WKEY_S) * 7/12}rem">
					<button class="rounded-full aspect-square {mask[n] ? 'bg-lime-600' : 'bg-gray-500'}" style:width="{(WKEY_W + WKEY_S) * 7/12*0.55}rem" on:click={() => maskButtonClicked(n)}></button>
				</div>
			{/each}
		</div>
	{/if}
