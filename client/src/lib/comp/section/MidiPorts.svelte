<script>
	...
	import SubsectionHeader from './comp/SubsectionHeader.svelte';
		...
		<div class="grid-cols-auto-1fr mx-auto grid max-w-[30rem] items-center gap-2 my-4">
			{#each $connectedMidiInputs as conName, i}
				...
				<div class="inline-block">
					<Select class="w-full rounded-full" options={[['', '--- None ---'], ...$availableMidiInputs]} value={conName ?? ''} on:change={e => updateSlot(i, e.target.value)} />
				</div>
			{/each}
		</div>
	</Content>
